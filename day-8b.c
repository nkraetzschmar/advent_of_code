int32_t scenic_score(uint8_t **map, uint64_t width, uint64_t height, uint64_t x, uint64_t y)
{
	uint8_t point_height = map[y][x];

	int32_t viewing_distance_left = 0;
	for (int64_t i = x-1; i >= 0; --i)
	{
		++viewing_distance_left;
		if (map[y][i] >= point_height) break;
	}

	int32_t viewing_distance_right = 0;
	...
	int32_t viewing_distance_up = 0;
	for (int64_t i = y-1; i >= 0; --i)
	{
		++viewing_distance_up;
		if (map[i][x] >= point_height) break;
	}

	int32_t viewing_distance_down = 0;
	...
	return viewing_distance_left * viewing_distance_right * viewing_distance_up * viewing_distance_down;
}
...
void init()
{
	...
	for (uint64_t x = 0; x < line_len - 1; ++x)
	{
		...
		{
			int32_t _score = scenic_score(map, line_len - 1, num_lines, x, y);
			if (_score > score) score = _score;
		}
	}

	uint8_t out_buf[0x100];
	uint64_t out_len = format_dec(score, out_buf, sizeof(out_buf));
	if (!out_len) exit(1);
	out_buf[out_len-1] = '\n';

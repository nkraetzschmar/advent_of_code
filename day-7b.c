...
uint64_t day_7(tree *t, uint64_t needed_size)
{
	if (t->type != directory) return 0;
	uint64_t size = recursive_size(t);
	if (size < needed_size) return 0;
	for (tree *child = t->directory.children; child; child = child->next)
	{
		uint64_t child_size = day_7(child, needed_size);
		if (child_size && child_size < size) size = child_size;
	}
	return size;
}
...
void init()
{
	...
	uint64_t used_size = recursive_size(root);
	uint64_t free_size = 70000000 - used_size;
	uint64_t needed_size = 30000000 - free_size;

	uint64_t size = day_7(root, needed_size);
